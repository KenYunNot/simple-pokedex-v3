...
export default function Slider({
  ...
  function moveLeft() {
    setHighlighted((highlighted + 6) % 7);
  }
  ...
  const arrowStyle = "text-white bg-gray-500 opacity-70 rounded-lg hover:opacity-100"
  return (
    <div className="slider-container">
      <button className="slider-button left" onClick={moveLeft}><ChevronLeftIcon className={arrowStyle} /></button>
      <button className="slider-button right" onClick={moveRight}><ChevronRightIcon className={arrowStyle} /></button>
      <ul
        className="slider"
        style={{
...
function CarouselItem({
  ...
  return (
    <li 
      className={clsx("border-r-4 border-gray-800", {
        "w-64 h-96 border-b-4" : !highlighted,
        "w-96 rounded-md" : highlighted,
        "clone" : clone,
      })} 
      key={pokemon.id}
    >
