...
const Pagination = ({
  ...
  return (
    <div className='flex justify-center w-fit mx-auto mt-6 mb-3 border rounded-lg overflow-hidden'>
      <PaginationArrow href={createPageUrl(currentPage-1)} direction='left' />
      {pagination.map((page, index) => {
        return (
          <React.Fragment key={index}>
            <PaginationNumber href={createPageUrl(page)} page={page} isCurrentPage={currentPage === page} />
          </React.Fragment>
        )
      })}
      <PaginationArrow href={createPageUrl(currentPage+1)} direction='right' />
    </div>
  )
}
...
export const PaginationNumber = ({
  ...
  return (
    <div className={cn('w-8 h-8 text-sm ', {
      'pointer-events-none' : typeof page === 'string',
      'hover:bg-gray-100' : typeof page === 'number',
      'bg-gray-400' : isCurrentPage,
    })}>
      {page === '...' ? page : <Link href={href} className='flex justify-center items-center w-full h-full'>{page}</Link>}
    </div>
  )
}
