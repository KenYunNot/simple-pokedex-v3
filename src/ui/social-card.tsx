    ...
    <div className={`group relative w-96 h-64 rounded-md ${backgroundColor}`}>
      ...
      <div className='relative w-full h-full rounded-md overflow-hidden'>
        <div className='flex items-center justify-center w-full h-full'>
          {match(social)
            .with('email', () => <MailIcon />)
            .otherwise(() => <SocialIcon url={`${social}.com`} className='shadow-[0_0_20px_5px] shadow-gray-500 rounded-full' />)}
        </div>
        <div className={`group-hover:-translate-y-full absolute w-full h-full z-10 duration-200 font-bold ${backgroundColor}`}>
          {match(social)
            .with('email', () => <p className='flex items-center justify-center h-full text-black pointer-events-none'>{href[social]}</p>)
            .otherwise(() => (
              <Link href={href[social]} className='group-hover:underline flex items-center justify-center h-full text-white' target='_blank'>
                {href[social]}
              </Link>
            ))}
        </div>
